...
class RXSNP()(implicit p: Parameters) extends L2Module {
    ...
    val implOpcodes        = Seq(SnpShared, SnpUnique, SnpCleanInvalid, SnpNotSharedDirty, SnpMakeInvalid, SnpOnce, SnpCleanShared) ++ { if (supportDCT) Seq(SnpSharedFwd, SnpNotSharedDirtyFwd, SnpUniqueFwd, SnpOnceFwd) else Nil }
