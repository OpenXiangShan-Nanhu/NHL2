...
class AtomicDataBufferWrite(implicit p: Parameters) extends L2Bundle {
    ...
    val bits = new Bundle {
        val data = Input(UInt(atomicDataBufferDataBits.W))
    }
...
class AtomicDataBufferRead(implicit p: Parameters) extends L2Bundle {
    ...
    val resp = Valid(UInt(chiBundleParams.dataBits.W))
}
...
class AtomicDataBuffer(implicit p: Parameters) extends L2Module {
    ...
    val dataBuffers = Reg(Vec(nrAtomicDataBuffer, UInt(atomicDataBufferDataBits.W)))
    ...
    io.read.resp.bits  := Fill(chiBundleParams.dataBits / atomicDataBufferDataBits, dataBuffers(rdIdxReg).asUInt)
